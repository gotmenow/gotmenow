    ...
    <>
      ...
      <section className="pt-32 sm:pt-40 pb-20">
        <div className="container-tight grid lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7">
            <div className="text-xs font-semibold tracking-[0.25em] uppercase text-[#C2410C] mb-6">About</div>
            <h1 className="font-display text-5xl sm:text-7xl leading-[1.02] text-[#0F172A]">
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-px bg-foreground/10 border border-foreground/10">
            {[
              ...
              <div key={c.t} className="bg-[#F8FAFC] p-8">
                <h3 className="font-display text-2xl text-[#0F172A] mb-3">{c.t}</h3>
      ...
      <section className="py-20 bg-[#0F172A] text-[#F8FAFC]">
        <div className="container-tight">
          ...
          <a href={INSTAGRAM_LINK} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-10 text-[#C2410C] font-medium hover:underline">
          ...
          <div className="mt-16 grid md:grid-cols-2 gap-px bg-foreground/10 border border-foreground/10">
            {TESTIMONIALS.map((t) => (
              <article key={t.name} className="bg-[#F8FAFC] p-8">
                <p className="text-lg text-[#0F172A] leading-relaxed">"{t.quote}"</p>
                <div className="mt-6 pt-6 border-t border-foreground/10">
                  <div className="font-semibold text-[#0F172A]">{t.name}</div>
                  <div className="text-sm text-[#475569]">{t.role}</div>
                  <div className="text-xs text-[#C2410C] mt-1">{t.result}</div>
                </div>
              </article>
            ))}
          </div>
