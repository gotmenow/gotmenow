    ...
    <>
      ...
      <section className="pt-32 sm:pt-40 pb-20">
        <div className="container-tight grid lg:grid-cols-2 gap-16">
          <div>
            <div className="text-xs font-semibold tracking-[0.25em] uppercase text-[#3d97e8] mb-6">Contact</div>
            <h1 className="font-display text-5xl sm:text-6xl leading-[1.02] text-[#0A2540]">Reach me directly.</h1>
            ...
            <div className="mt-10 space-y-5">
              <a href={`mailto:${EMAIL}`} className="flex items-center gap-4 group">
                <div className="w-12 h-12 rounded-sm border border-foreground/15 flex items-center justify-center group-hover:bg-[#0A2540] group-hover:text-white transition-colors">
                  <Mail className="w-5 h-5" />
                </div>
                <div>
                  <div className="text-xs text-[#475569] uppercase tracking-widest">Email</div>
                  <div className="font-display text-xl text-[#0A2540]">{EMAIL}</div>
                </div>
              </a>
              <a href={INSTAGRAM_LINK} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 group">
                ...
                <div>
                  ...
                  <div className="font-display text-xl text-[#0A2540]">{INSTAGRAM_HANDLE}</div>
                </div>
              </a>
              <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 group">
                ...
                <div>
                  ...
                  <div className="font-display text-xl text-[#0A2540]">Join the community</div>
                </div>
              </a>
            </div>
          </div>
          ...
          <div>
            <div className="bg-white border border-foreground/10 p-8 sm:p-10">
              <h2 className="font-display text-2xl text-[#0A2540] mb-6">Send a quick message</h2>
              {status === "done" ? (
                <div className="py-10 text-center">
                  <p className="text-lg text-[#0A2540] font-display">Message sent. I'll be in touch.</p>
                </div>
              ) : (
                <form onSubmit={submit} className="space-y-5">
                  ...
                  {status === "error" && <p className="text-sm text-[#3d97e8]">Something went wrong. Please try WhatsApp instead.</p>}
