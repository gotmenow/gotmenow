...
export default function SectionHeader({ number, eyebrow, title, intro, align = "left", dark = false }) {
  ...
  return (
    <div ref={ref} className={`relative ${align === "center" ? "text-center mx-auto max-w-2xl" : "max-w-3xl"}`}>
      ...
      {eyebrow && (
        <div className={`text-xs font-semibold tracking-[0.2em] uppercase text-[#00a99d] mb-4 ${align === "center" ? "flex items-center justify-center gap-2" : ""}`}>
          {eyebrow}
        </div>
      )}
      <h2 className={`font-display text-4xl sm:text-5xl lg:text-6xl leading-[1.05] ${dark ? "text-[#F8FAFC]" : "text-[#000000]"}`}>
        {title}
      </h2>
